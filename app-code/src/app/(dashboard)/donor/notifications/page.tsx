...
type FilterTab = 'ALL' | 'UNREAD' | 'DONATIONS' | 'MESSAGES';
...
const typeStyles: Record<string, { icon: string; color: string; bg: string }> = {
    ...
    RECEIPT: { icon: 'receipt_long', color: '#0ea5e9', bg: 'rgba(14,165,233,0.1)' },
    TAX_DOCUMENT: { icon: 'description', color: '#8b5cf6', bg: 'rgba(139,92,246,0.1)' },
    THANK_YOU: { icon: 'favorite', color: '#ec4899', bg: 'rgba(236,72,153,0.1)' },
    NGO_UPDATE: { icon: 'campaign', color: '#f59e0b', bg: 'rgba(245,158,11,0.1)' },
    MESSAGE: { icon: 'chat', color: '#3b82f6', bg: 'rgba(59,130,246,0.1)' },
    VERIFICATION: { icon: 'verified', color: '#3b82f6', bg: 'rgba(59,130,246,0.1)' },
    VOLUNTEER: { icon: 'diversity_3', color: '#1de2d1', bg: 'rgba(29,226,209,0.1)' },
...
export default function DonorNotificationsPage() {
    ...
    const filtered = notifications.filter(n => {
        ...
        if (activeTab === 'DONATIONS') return ['DONATION', 'RECEIPT', 'TAX_DOCUMENT', 'THANK_YOU'].includes(n.notification_type);
    ...
    const tabs: { key: FilterTab; label: string; icon: string }[] = [
        ...
        { key: 'DONATIONS', label: 'Donations', icon: 'payments' },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    backgroundImage: 'radial-gradient(circle at 20% 50%, #10b981 0%, transparent 50%), radial-gradient(circle at 80% 50%, #fbbf24 0%, transparent 50%)',
                }} />
                <div style={{ position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                        ...
                        <Link href="/donor/notifications/settings" style={{
                            display: 'flex', alignItems: 'center', gap: 6, padding: '10px 18px',
                            borderRadius: 12, border: '1px solid rgba(255,255,255,0.15)',
                            background: 'rgba(255,255,255,0.08)', backdropFilter: 'blur(8px)',
            ...
            <div style={{
                ...
                {tabs.map(tab => (
                    <button key={tab.key} onClick={() => setActiveTab(tab.key)} style={{
                        ...
                        border: activeTab === tab.key ? '1.5px solid #10b981' : '1px solid #e2e8f0',
                        background: activeTab === tab.key ? 'rgba(16,185,129,0.08)' : '#fff',
            ...
            {loading ? (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 200 }}>
                    <span className="material-symbols-outlined animate-spin" style={{ fontSize: 32, color: '#10b981' }}>progress_activity</span>
                </div>
            ) : filtered.length === 0 ? (
                <div style={{
                    ...
                    <p style={{ color: '#94a3b8', marginTop: 6, fontSize: 14, maxWidth: 320, margin: '6px auto 0' }}>
                        {activeTab === 'UNREAD'
                            ...
                            : 'When you receive donation confirmations, receipts, or update from NGOs, they will show up here.'}
                    </p>
                    {activeTab !== 'ALL' && (
                        ...
                        return (
                            <div key={n.id}
                                style={{
                                    ...
                                    background: n.is_read ? '#fff' : 'rgba(16,185,129,0.03)',
                                    border: `1px solid ${n.is_read ? '#f1f5f9' : 'rgba(16,185,129,0.3)'}`,
                                    boxShadow: n.is_read ? 'none' : '0 0 0 1px rgba(16,185,129,0.05)',
                                }}
                                onClick={() => !n.is_read && markAsRead(n.id)}
                                onMouseEnter={e => e.currentTarget.style.boxShadow = '0 4px 20px rgba(0,0,0,0.06)'}
                                onMouseLeave={e => e.currentTarget.style.boxShadow = n.is_read ? 'none' : '0 0 0 1px rgba(16,185,129,0.05)'}
                            >
                                {/* Icon */}
                                <div style={{

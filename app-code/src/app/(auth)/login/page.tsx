...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (profileError) {
                console.warn('[Login] Profile query failed:', profileError.message);
                // User row missing — try to create it so future logins work correctly
                const metadataRole = result.user.user_metadata?.role || 'INDIVIDUAL';
                try {
                    await fetch('/api/auth/ensure-user', {
                        ...
                        body: JSON.stringify({
                            ...
                            full_name: result.user.user_metadata?.full_name || 'User',
                            role: metadataRole,
                        }),
                    });
                    console.log('[Login] Created missing user row with role:', metadataRole);
                } catch (ensureErr) {
                    console.error('[Login] Failed to create user row:', ensureErr);
                }
            }

            // Use profile role, fallback to auth metadata role, then to DONOR
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <button
                    ...
                    style={{
                        ...
                        height: 44, borderRadius: 8, border: "1px solid #e2e8f0", background: "#f8fafc",
                        fontSize: 14, fontWeight: 600, color: "#94a3b8", cursor: "not-allowed", opacity: 0.6,
                    }}
                >
